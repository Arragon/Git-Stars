...
import { PendingMutations } from "../components/PendingMutations";
...
export const Dashboard: React.FC = () => {
  ...
  const [showSyncPanels, setShowSyncPanels] = useState(false);
  ...
  const [collectionProjects, setCollectionProjects] = useState<
    CollectionProject[]
  >([]);
  ...
  const [isCollectionComposerOpen, setIsCollectionComposerOpen] =
    useState(false);
  const [isSavingCollection, setIsSavingCollection] = useState(false);
  const [editingCollectionName, setEditingCollectionName] = useState("");
  const [editingCollectionDescription, setEditingCollectionDescription] =
    useState("");
  const [collectionAutoCollectEnabled, setCollectionAutoCollectEnabled] =
    useState(false);
  const [selectedProjectIds, setSelectedProjectIds] = useState<string[]>([]);
  const [runningAutoCollectionId, setRunningAutoCollectionId] = useState<
    string | null
  >(null);
  ...
  const mergeCollectionProjects = useCallback(
    (records: CollectionProject[]) => {
  ...
  }, [setIsLoadingData, setProjects, user, loadFromCache]);
  ...
  const loadCollections = useCallback(async () => {
    ...
    try {
      const data = await apiGet<unknown[]>("/api/collections");

      const nextCollections = (data || []).map((item) =>
  ...
  const runAutoCollect = useCallback(
    async (collection: Collection, silent: boolean = false) => {
      ...
      try {
        ...
        const response = await apiPost<{ items: unknown[] }>(
          "/api/collections/-projects",
          { items: payload },
        );

        mergeCollectionProjects(
          (response.items || []).map((item) =>
            normalizeCollectionProject(item as Record<string, unknown>),
          ),
        );

        if (!silent) {
          alert(
            `Auto collected ${payload.length} project(s) into "${collection.name}".`,
          );
        }
        ...
        if (!silent) {
          alert(
            "Auto collect failed. Please try again after generating AI summaries.",
          );
        }
        return 0;
      } finally {
        setRunningAutoCollectionId((current) =>
  ...
  useEffect(() => {
    if (
      ...
      autoCollectBootstrappedRef.current
    ) {
      return;
    }
    ...
    const enabledCollections = collections.filter(
      (collection) => collection.auto_collect_enabled,
    );
  ...
  const collectionsById = useMemo(() => {
    return collections.reduce<Record<string, Collection>>(
  ...
  const projectCollectionsMap = useMemo(() => {
    return collectionProjects.reduce<Record<string, Collection[]>>(
      (accumulator, item) => {
  ...
  const collectionCounts = useMemo(() => {
    const counts: Record<string, number> = {
      ...
      [GITHUB_STARS_COLLECTION_ID]: projects.filter(
        (project) => project.type === "star",
      ).length,
    };
  ...
  useEffect(() => {
    ...
    setCollectionAutoCollectEnabled(
      Boolean(selectedCollection.auto_collect_enabled),
    );
  }, [selectedCollection]);
  ...
  const handleSync = async (isAutoSync: boolean | React.MouseEvent = false) => {
    ...
    const { setPullStatus, setPullError } = useSyncStatusStore.getState();

    try {
      // Pull from server change feed
      setPullStatus("pulling");
      await pullSync.sync();

      // Push any pending local mutations
      ...
      setPullStatus("error");
      setPullError(err instanceof Error ? err.message : "Sync failed");
      if (!isAuto) {
        alert(
          "Failed to sync data. This may be due to network issues or rate limits. Please try again later.",
        );
      }
    }
  ...
  const handleAutoSummarize = async (forceAll: boolean = false) => {
    ...
    const projectsToSummarize = forceAll
      ? projects
      : projects.filter((project) => !project.ai_summary);
    ...
    const allExistingTags = forceAll
      ? []
      : Array.from(
          new Set(projects.flatMap((project) => project.ai_tags || [])),
        );
    ...
    for (let index = 0; index < projectsToSummarize.length; index++) {
      ...
      setSummarizeProgress({
        current: index + 1,
        total: projectsToSummarize.length,
      });
      ...
      try {
        ...
        await apiPatch(`/api/projects/${project.id}/ai`, {
        ...
        setProjects(
          useDashboardStore.getState().projects.map((item) =>
            item.id === project.id
              ? {
                  ...item,
        ...
        if (
          index < projectsToSummarize.length - 1 &&
          !stopSummarizeRef.current
        ) {
    ...
    if (stoppedByUser) {
      console.log(
        `Auto summarize paused. Processed ${successCount} projects this run.`,
      );
      return;
    }

    alert(
      `Auto summarize complete! Successfully processed ${successCount} projects.`,
    );
  ...
  const handleCreateCollection = async () => {
    ...
    try {
      const data = await apiPost<Record<string, unknown>>("/api/collections", {
        name: trimmedName,
  ...
  const handleDeleteSelectedCollection = async () => {
    ...
    setIsSavingCollection(true);
    try {
      await apiDelete(`/api/collections/${selectedCollection.id}`);

      setCollections((current) =>
        current.filter((collection) => collection.id !== selectedCollection.id),
      );
      setCollectionProjects((current) =>
        current.filter((item) => item.collection_id !== selectedCollection.id),
      );
      setSelectedCollectionId(ALL_PROJECTS_COLLECTION_ID);
  ...
  const handleToggleProjectCollection = async (
    projectId: string,
    ...
    try {
      ...
      await apiDelete("/api/collections/-projects", {
        collection_id: collection.id,
      ...
      console.error(
        "[Dashboard] Failed to update collection membership:",
        error,
      );
      ...
      .filter((project) => {
        ...
        if (searchQuery) {
          ...
          const matchesSearch =
            ...
            (project.full_name &&
              project.full_name.toLowerCase().includes(searchLower)) ||
            (project.description &&
              project.description.toLowerCase().includes(searchLower)) ||
            (project.ai_summary &&
              project.ai_summary.toLowerCase().includes(searchLower)) ||
            (project.ai_tags || []).some((tag) =>
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "name") {
          ...
          const dateA = new Date(
            a.starred_at || a.github_created_at || 0,
          ).getTime();
          const dateB = new Date(
            b.starred_at || b.github_created_at || 0,
          ).getTime();
  ...
  const handleToggleSelectAllVisible = () => {
    ...
    const allVisibleSelected =
      visibleIds.length > 0 &&
      visibleIds.every((id) => selectedProjectIds.includes(id));
  ...
  const handleBatchRemoveFromCollection = async () => {
    ...
    if (
      !confirm(
        `Remove ${selectedProjectIds.length} project(s) from "${selectedCollection.name}"?`,
      )
    ) {
      return;
    }
    ...
    <div className="flex h-[calc(100vh-64px)] w-full overflow-hidden">
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <button
                type="button"
                onClick={() =>
                  setSelectedCollectionId(GITHUB_STARS_COLLECTION_ID)
                }
                className={`w-full flex items-center justify-between rounded-lg border px-3 py-2 text-left transition-colors ${
                  selectedCollectionId === GITHUB_STARS_COLLECTION_ID
            ...
            {collections.length === 0 && (
              <p className="mt-3 text-sm text-gray-400 italic">
                Create your first custom collection to organize projects.
              </p>
            )}
                ...
                <div className="flex flex-wrap gap-2">
                  {allTags.map((tag) => {
                    ...
                    return (
                      ...
                      >
                        <Tag
                          className={`w-3 h-3 mr-1 ${isSelected ? colors.text : "text-gray-400"}`}
                        />
                        {tag}
                      </button>
                    );
              ...
              <div>
                ...
                <div className="space-y-3">
                    ...
                    onChange={(event) =>
                      setSortBy(
                        event.target.value as
                          "starred_at" | "stars_count" | "name" | "activity",
                      )
                    }
                    className="block w-full pl-3 pr-10 py-2 text-base border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md bg-white cursor-pointer"
                  >
                  ...
                  >
                    ...
                    <Filter
                      className={`h-4 w-4 text-gray-400 transition-transform duration-200 ${sortOrder === "desc" ? "transform rotate-180" : ""}`}
                    />
                  </button>
                </div>
              </div>
      ...
      <div className="flex-1 overflow-y-auto bg-white">
        <div className="p-6 md:p-8 lg:p-10 w-full mx-auto space-y-8">
              ...
              <div className="flex gap-3 relative group">
                <div className="flex rounded-lg shadow-sm">
                  <button
                    ...
                    title={
                      isAutoSummarizing
                        ? "Pause automatic summarization"
                        : "Automatically summarize all projects without a summary"
                    }
                  >
                    {isAutoSummarizing ? (
                      <>
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        <span>
                          Pause ({summarizeProgress.current}/
          ...
          {selectedCollectionId === GITHUB_STARS_COLLECTION_ID && (
            <div className="rounded-2xl border border-yellow-200 bg-yellow-50/70 p-5">
              ...
              <p className="mt-2 text-sm text-yellow-800">
                This smart collection mirrors repositories already starred in
                your GitHub account.
              </p>
            </div>
          )}
          ...
          {selectedCollection && (
            <div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-5">
              <div className="grid grid-cols-1 xl:grid-cols-[2fr_2fr_auto] gap-4 items-start">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      setEditingCollectionDescription(event.target.value)
                    }
                    rows={3}
                    className="block w-full rounded-md border border-blue-100 bg-white px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Describe what belongs in this collection..."
                ...
                <div className="flex flex-col gap-2">
                  <label className="inline-flex items-center gap-2 rounded-md border border-blue-100 bg-white px-3 py-2 text-sm text-gray-700">
                    <input
                      ...
                      onChange={(event) =>
                        setCollectionAutoCollectEnabled(event.target.checked)
                      }
                  ...
                  >
                    {runningAutoCollectionId === selectedCollection.id ? (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ) : (
                      <Sparkles className="mr-2 h-4 w-4" />
                    )}
              ...
              <p className="mt-3 text-sm text-blue-800">
                {collectionCounts[selectedCollection.id] || 0} project(s)
                currently in this collection. Auto collect uses project AI
                summaries and tags to find related repositories.
              </p>
            </div>
          )}
            ...
            <div className="flex flex-wrap items-center gap-2 pb-2 border-b border-gray-100">
              ...
              {filterTag !== "all" &&
                (() => {
                  ...
                  return (
                    <span
                      className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium ${colors.bg} ${colors.text} ring-1 ring-inset ${colors.border}`}
                    >
                      Tag: {filterTag}
                      <button
                        onClick={() => setFilterTag("all")}
                        className={`ml-1.5 ${colors.text} hover:opacity-75`}
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </span>
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
                <div>
                  <p className="text-sm font-semibold text-amber-900">
                    Manage projects in this collection
                  </p>
                  <p className="text-sm text-amber-800">
                    {selectedProjectIds.length} selected. Use checkboxes on
                    cards for batch remove.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {filteredAndSortedProjects.length > 0 &&
                    filteredAndSortedProjects.every((project) =>
          ...
          <button
            type="button"
            onClick={() => setShowSyncPanels((v) => !v)}
            className="text-xs text-gray-500 hover:text-gray-700 underline"
          >
            {showSyncPanels ? "Hide sync details" : "Show sync details"}
          </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
              {filteredAndSortedProjects.map((project) => (
                <ProjectCard
                  ...
                  onToggleCollection={(collection, shouldInclude) =>
                    handleToggleProjectCollection(
                      project.id,
                      collection,
                      shouldInclude,
                    )
                  ...
                  onRemoveFromCurrentCollection={
                    isCustomCollectionSelected
                      ? handleRemoveFromCurrentCollection
                      : undefined
                  }
