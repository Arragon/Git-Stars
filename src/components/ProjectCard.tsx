...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const { projects, setProjects, filterTag, setFilterTag } =
    useDashboardStore();
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      const allExistingTags = Array.from(
        new Set(projects.flatMap((p) => p.ai_tags || [])),
      );
  ...
  return (
    <div
      className={`bg-white rounded-xl border ${isCurrentlySummarizing ? "border-purple-400 ring-1 ring-purple-400 shadow-md" : "border-gray-200"} shadow-sm hover:shadow-md transition-all duration-200 flex flex-col h-full text-sm hover:-translate-y-0.5 relative overflow-hidden`}
    >
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center space-x-2.5 truncate pr-2">
            ...
            <BookMarked
              className={`w-4.5 h-4.5 flex-shrink-0 ${isCurrentlySummarizing ? "text-purple-500" : "text-gray-400"}`}
            />
            <h3
          ...
          <div className="flex items-center space-x-1.5 flex-shrink-0">
            {onToggleCollection && (
              <CollectionPicker
                ...
                assignedCollectionIds={assignedCollections.map(
            ...
            <button
              ...
              title={
                displaySummary ? "Re-summarize with AI" : "Summarize with AI"
              }
            >
              {isCurrentlySummarizing ? (
        ...
        <p
          className="text-gray-600 mb-3 line-clamp-2 leading-relaxed text-sm flex-1"
        ...
        {displaySummary && (
          <div
            className={`mb-3 p-3 bg-purple-50/80 rounded-lg text-xs text-purple-900 border ${isCurrentlySummarizing ? "border-purple-300" : "border-purple-100/60"} leading-relaxed shadow-sm transition-colors`}
          >
        ...
        <div
          className={`flex flex-wrap items-center gap-3.5 text-xs font-medium text-gray-500 ${!displayTags?.length && !displaySummary && !isCurrentlySummarizing ? "mt-auto" : ""}`}
        >
          {project.language && (
      ...
      <div className="px-5 py-3 bg-gray-50/50 border-t border-gray-100 flex justify-between items-center rounded-b-xl gap-3">
        <div
          ...
          title={project.type === "star" ? "Starred on" : "Forked on"}
        >
          <Calendar className="w-3.5 h-3.5 mr-1.5" />
          <span>
            {project.type === "star" ? "Starred" : "Forked"}{" "}
            {formatDate(project.starred_at || project.github_created_at)}
          </span>
        </div>
